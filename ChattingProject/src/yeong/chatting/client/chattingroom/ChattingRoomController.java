...
public class ChattingRoomController extends BaseController{
	...
	private void send() {
		...
		if(whisper.getSelectionModel().getSelectedIndex() == 0) {
			...
			secretSend.setCons(whisper,chat);
			action(secretSend);
		}
	}
	...
	public void setWhisper(Member m, String to, String msg) {
		StringBuilder sb = new StringBuilder();
		SimpleDateFormat sdf = new SimpleDateFormat("HH:mm");
		String time = sdf.format(Calendar.getInstance().getTime());
				...
				ListCell<Member> cell = new ListCell<Member>() {
					...
					protected void updateItem(Member item, boolean empty) {
						if(item != null && !empty) {
							...
							if(item.getName().equals(ClientInfo.currentRoom.getRoom_host())) {
								setText(sb.toString());
								return;
							}
	...
	private void initWhisperList() {
		...
		whisper.setConverter(new StringConverter<Member>() {
